    ...
    <v-card outlined class="d-flex flex-row">
      ...
      >
        <div style="width: 100px">
          ...
          />
          <label id="reps" style="display: none">Repeticiones de ciclo</label>
        </div>
      </div>
      <div style="padding-bottom: 40px; min-width: 600px" class="ml-5">
        <div class="d-flex flex-row justify-space-between align-baseline">
          ...
          <v-btn
            ...
            color="primary"
            :disabled="
              this.cycleType !== CycleTypes.EXERCISE || this.order === 2
            "
            @click="deleteDialog = true"
          >
            <v-icon>mdi-delete</v-icon>
            <div style="display: none">Borrar ciclo</div>
          </v-btn>
        </div>
        <v-divider></v-divider>
        <v-card-text>
          <div class="d-flex flex-row justify-center">
            <v-btn icon @click="addExercise(0)" color="primary">
              ...
              <div style="display: none">Agregar ejercicio</div>
            </v-btn>
          </div>
          <v-divider></v-divider>
          <div
...
export default {
  ...
  async mounted() {
    ...
    if (this.cycleId !== undefined && this.cycleId !== null) {
      let cycle = await useCycles().getCycleWithExercises(
        this.routineId,
        this.cycleId
      );
  ...
  methods: {
    ...
    sumbitEx(ExerciseSelected, ExerciseType, index) {
      this.exercises.splice(index, 0, {
        ...
        img_url:
          ExerciseSelected.images.length > 0
            ? ExerciseSelected.images[0].url
            : abspic
      });
